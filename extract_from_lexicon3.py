...
def get_phenomena():
    ...
    # getting prepositions (NP attached)
    word_lfs = get_prep_lfs()
    # phenomena.append(Phenomenon('Prepositions', word_lfs, '(PP/NP)', \
    #                                 ['(PP/NP)', "(PP\NP)"],sem_type='(PP|NP)'))
    phenomena.append(Phenomenon('Prepositions NP', word_lfs, '((NP\NP))/NP)', \
                               ['((NP/NP)/NP)', '((NP\NP)\NP)', '((NP\NP)/NP)', '((NP/NP)\NP'], sem_type='((NP|NP)|NP)'))

    # getting prepositions (VP attached)
    # word_lfs = get_prep_lfs()
    # # phenomena.append(Phenomenon('Prepositions', word_lfs, '(PP/NP)', \
    # #                                 ['(PP/NP)', "(PP\NP)"],sem_type='(PP|NP)'))
    # phenomena.append(Phenomenon('Prepositions VP', word_lfs, '((NP\NP))/NP)', \
    #                            ['((NP/NP)/NP)', '((NP\NP)\NP)', '((NP\NP)/NP)', '((NP/NP)\NP'], sem_type='((NP|NP)|NP)'))
...
def get_prep_vp_lfs():
    ...
    for line in f:
        fields = line.strip().split(' ')
        for k in ["lambda $0_{e}.lambda $1_{<r,t>}.lambda $2_{r}.and($1($2), " + fields[1] + "($0,$2)"]:
            prep_to_lf.append( (fields[0],k) )
    f.close()
    return prep_to_lf
